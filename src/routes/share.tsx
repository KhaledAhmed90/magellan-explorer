  ...
  head: () => ({
    meta: [
      { title: "Share Location — Magellan" },
      {
        name: "description",
        content:
          "Generate a real scannable QR code for the versioned Magellan location payload and pick a transport.",
      },
...
function SharePage() {
  ...
  const payload: LocationPayloadV1 | null = useMemo(() => {
    if (sourceId === "live") {
      ...
      return {
        ...
        alt: snapshot.altitudeM ? Number(snapshot.altitudeM.toFixed(1)) : undefined,
        acc: snapshot.accuracyM ? Number(snapshot.accuracyM.toFixed(1)) : undefined,
  ...
  const encoded = payload ? encodeLocationPayload(payload) : "";
  const active = TRANSPORTS.find((tr) => tr.id === transport)!;
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        title={t("nav_share")}
        description="One versioned payload (MGLN v1) travels over every transport."
        actions={<DemoBadge label="DEMO PAYLOAD" />}
      />
      <SimulatedNotice>
        The QR image below is genuinely generated and scannable. The coordinates inside it come
        from demo data, not from a real GNSS fix.
      </SimulatedNotice>
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <div className="space-y-4 rounded-lg border border-border bg-card p-4">
          ...
          <div className="space-y-2">
            ...
            <Input id="note" value={note} onChange={(e) => setNote(e.target.value)} maxLength={160} />
          </div>
            ...
            <div className="grid gap-2">
              {TRANSPORTS.map((tr) => (
                <button
                  ...
                  className={cn(
                    "grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 rounded-md border p-2 text-start",
                    transport === tr.id ? "border-primary bg-secondary" : "border-border",
                  )}
                >
          ...
          <Button className="w-full" onClick={handleSend} disabled={!payload}>
            Send via {active.name}
          </Button>
        </div>
        ...
        <div className="space-y-3 rounded-lg border border-border bg-card p-4">
          ...
          {encoded ? (
            <>
              <QrCanvas value={encoded} />
              <div className="rounded-md border border-border bg-muted/40 p-3">
                <div className="label-eyebrow">Encoded payload</div>
                <pre className="numeric mt-1 overflow-x-auto text-[11px] whitespace-pre-wrap break-all">
                  {encoded}
                </pre>
              </div>
              <Button
                ...
                onClick={() => {
                  ...
                  toast.success("Payload copied — paste it in Receive location");
                }}
              >
                Copy payload
              </Button>
            ...
            <p className="text-sm text-muted-foreground">
              No position available, so no QR can be generated.
            </p>
          )}
        </div>
      </div>
