  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <header className="sticky top-0 z-30 border-b border-border bg-background/90 backdrop-blur">
        <div className="mx-auto grid max-w-6xl grid-cols-[minmax(0,1fr)_auto] items-center gap-3 px-4 py-3">
          ...
          <div className="flex shrink-0 items-center gap-1">
            <DemoBadge label="WEB PROTOTYPE" className="hidden sm:inline-flex" />
            <Button
              variant="ghost"
              size="icon"
      ...
      <div className="mx-auto flex max-w-6xl gap-6 px-4 py-5">
        <nav className="hidden w-56 shrink-0 lg:block" aria-label="Primary">
          <div className="sticky top-20 space-y-5">
            {GROUPS.map((g) => (
              <div key={g.title} className="space-y-1">
                <div className="label-eyebrow px-2">{t(g.title)}</div>
                {g.items.map((item) => (
                  <Link
                    key={item.to}
                    to={item.to}
